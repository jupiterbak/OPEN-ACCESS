    ...
    self.server = net.createServer(function (socket) {
        ...
        socket.on('data', function (data) {
             try {
				...
				self.temp_Buffer = Buffer.concat([self.temp_Buffer,data],self.temp_Buffer.length + data.byteLength);
                if(self.temp_Buffer.length >= MAX_PROGRAM_DATA_LENGTH){
					if( self.temp_Buffer.length == MAX_PROGRAM_DATA_LENGTH){
						self.parseProgramm(self.temp_Buffer, socket.name);
...
tcp_server.prototype.mainClientWrite = function (prgObj) {
    ...
    if(self.main_client){
		...
		var l = prgObj.length;
		this.writeBuffer = new Buffer((40 * l) + 8);
		this.writeBuffer.writeUInt32BE(l, 0);
        for(k=0;k < l; k++){
            var _index = k * 40;
            self.encodeCmd(prgObj[k],self.writeBuffer,_index + 4);
        }		
